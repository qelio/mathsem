...
#ifdef _UNICODE
CXMLArchiveNode* CXMLArchive::GetNode(LPCSTR nodeNameStr)
{
	wchar_t nodeNameWStr[512];
	mbstowcs(nodeNameWStr, nodeNameStr, ARRAYSIZE(nodeNameWStr));
	return ( CXMLArchive::GetNode(nodeNameWStr) );
}
#endif
...
void CXMLArchiveNode::DataNode(LPCTSTR attrName, CTimeSpan& attrValue)
{
	...
	else
	{
		...
		int noFields = _stscanf(tmpString, _T("%d %d:%d:%d"), &d, &h, &mm, &s);
...
void CXMLArchiveNode::DataNode(LPCTSTR attrName, CTime& attrValue)
{
	...
	else
	{
		...
		int noFields = _stscanf(tmpString, _T("%d/%d/%d %d:%d:%d"), &d, &m, &y, &h, &mm, &s);
